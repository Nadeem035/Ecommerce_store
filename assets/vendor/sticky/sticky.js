...
(function ($) {
	...
	$.fn.themePin = function (options) {
		...
		this.each(function () {
			...
			$( "img", this ).one( "load", function () {
				if ( r_timer ) {
					AMZSTORE.deleteTimeout( r_timer );
				}
				r_timer = AMZSTORE.requestTimeout( recalculateLimits, 20 );
			} );
			data.update = update;
	...
	Sticky.prototype = {
		...
		build: function () {
			...
			setTimeout(function() {
				if ( typeof AMZSTORE.sticky_top_height != 'undefined' && $el.data('themePin').pad ) {
					$el.data('themePin').pad.top = AMZSTORE.sticky_top_height;
				}
			}, 300);
